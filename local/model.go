...
func storeRecord(UID string, r *http.Request) (bool) {
	...
	if err == nil {
		...
		for index, fileHeader := range fileHeaders {
			originalFile, openFileErr := fileHeader.Open()
			defer originalFile.Close()
			checkErr(openFileErr, "open photo file err")
			if openFileErr == nil {
				//fmt.Println("filename:", fileHeader.Filename, " bytes:", fileHeader.Size)
				randString := newRandomString(35)
				photoExt := filepath.Ext(fileHeader.Filename)
				photoPath := randString + photoExt

				newFile, createPhotoFileErr := os.Create(frogConfig.StoragePath + "photo/" + photoPath)
				defer newFile.Close()
				...
				if copyErr == nil {
					if decodeErr != nil {
						_, storeRecordPhotoErr := db.Exec("INSERT INTO photo SET userid=?, recordid=?, initorder=?, path=?, name=?, createtime=CURRENT_TIMESTAMP", UID, recordID, index, photoPath, fileHeader.Filename)
						...
						_, storeRecordPhotoErr := db.Exec("INSERT INTO photo SET userid=?, recordid=?, initorder=?, path=?, name=?, longitude=?, latitude=?, altitude=?, shootdatetime=?, createtime=CURRENT_TIMESTAMP", UID, recordID, index, photoPath, fileHeader.Filename, longitude, latitude, altitude, dateAndTime)
						checkErr(storeRecordPhotoErr, "store record photo err")
						if storeRecordPhotoErr == nil {
							successStore = true
						}
